...
const CannonBall3D = () => {
  ...
  const [compliance, setCompliance] = useState(0.0);
  useEffect(() => {
    if (canvasRef.current) {
      const threeScene = new ThreeScene(canvasRef.current);
      threeScene.initGrabber();
      threeScene.addDragonMesh();
      threeScene.camera?.position.set(0, 1.5, 3);
  ...
  useEffect(() => {
    if (threeScene) {
      for (let i = 0; i < threeScene.physicsScene.objects.length; i++)
        threeScene.physicsScene.objects[i].edgeCompliance = compliance * 10.0;
    }
  }, [compliance]);
