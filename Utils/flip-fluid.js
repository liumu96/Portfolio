...
class FlipFluid {
  ...
  updateParticleDensity(f) {
    ...
    if (this.particleRestDensity == 0.0) {
      ...
      for (var i = 0; i < this.fNumCells; i++) {
        if (this.cellType[i] == this.FLUID_CELL) {
          sum += d[i];
          numFluidCells++;
        }
      }
    ...
    // 					if (this.cellType[(xi - 1) * n + yi] == this.SOLID_CELL || this.cellType[(xi + 1) * n + yi] == this.SOLID_CELL)
    // 						hx -= this.particleRadius;
    // 					if (this.cellType[xi * n + yi - 1] == this.SOLID_CELL || this.cellType[xi * n + yi + 1] == this.SOLID_CELL)
  ...
  transferVelocities(toGrid, flipRatio) {
    ...
    if (toGrid) {
      ...
      for (var i = 0; i < this.fNumCells; i++)
        this.cellType[i] = this.s[i] == 0.0 ? this.SOLID_CELL : this.AIR_CELL;
      ...
      for (var i = 0; i < this.numParticles; i++) {
        ...
        var cellNr = xi * n + yi;
        if (this.cellType[cellNr] == this.AIR_CELL)
          this.cellType[cellNr] = this.FLUID_CELL;
      }
    }
    ...
    for (var component = 0; component < 2; component++) {
      ...
      for (var i = 0; i < this.numParticles; i++) {
        ...
        if (toGrid) {
          ...
          var valid0 =
            this.cellType[nr0] != this.AIR_CELL ||
            this.cellType[nr0 - offset] != this.AIR_CELL
              ? 1.0
              : 0.0;
          var valid1 =
            this.cellType[nr1] != this.AIR_CELL ||
            this.cellType[nr1 - offset] != this.AIR_CELL
              ? 1.0
              : 0.0;
          var valid2 =
            this.cellType[nr2] != this.AIR_CELL ||
            this.cellType[nr2 - offset] != this.AIR_CELL
              ? 1.0
              : 0.0;
          var valid3 =
            this.cellType[nr3] != this.AIR_CELL ||
            this.cellType[nr3 - offset] != this.AIR_CELL
              ? 1.0
              : 0.0;
      ...
      if (toGrid) {
        ...
        for (var i = 0; i < this.fNumX; i++) {
          for (var j = 0; j < this.fNumY; j++) {
            var solid = this.cellType[i * n + j] == this.SOLID_CELL;
            if (
              solid ||
              (i > 0 && this.cellType[(i - 1) * n + j] == this.SOLID_CELL)
            )
              this.u[i * n + j] = this.prevU[i * n + j];
            if (
              solid ||
              (j > 0 && this.cellType[i * n + j - 1] == this.SOLID_CELL)
            )
    ...
    for (var iter = 0; iter < numIters; iter++) {
      for (var i = 1; i < this.fNumX - 1; i++) {
        for (var j = 1; j < this.fNumY - 1; j++) {
          if (this.cellType[i * n + j] != this.FLUID_CELL) continue;
  ...
  updateCellColors() {
    ...
    for (var i = 0; i < this.fNumCells; i++) {
      if (this.cellType[i] == this.SOLID_CELL) {
        this.cellColor[3 * i] = 0.5;
        this.cellColor[3 * i + 1] = 0.5;
        this.cellColor[3 * i + 2] = 0.5;
      } else if (this.cellType[i] == this.FLUID_CELL) {
